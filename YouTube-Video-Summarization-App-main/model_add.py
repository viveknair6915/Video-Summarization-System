...
class LlamaCPPInvocationLayer(PromptModelInvocationLayer):
    ...
    def _ensure_token_limit(self, prompt: Union[str, List[Dict[str, str]]]) -> Union[str, List[Dict[str, str]]]:
        ...
        max_tokens_for_summary = int(len(tokenized_prompt) * 0.3)
        
        if len(tokenized_prompt) > max_tokens_for_summary:
            logger.warning(
                "The prompt has been truncated from %s tokens to %s tokens to fit the target summary length (%s tokens). "
                "Consider adjusting the prompt for better results.",
                len(tokenized_prompt),
                max_tokens_for_summary,
                max_tokens_for_summary
            )
            return bytes.decode(self.model.detokenize(tokenized_prompt[:max_tokens_for_summary]), 'utf-8')
    ...
    def invoke(self, *args, **kwargs):
        ...
        if kwargs and "prompt" in kwargs:
            ...
            model_input_kwargs["max_tokens"] = int(self.max_length * 0.3)
